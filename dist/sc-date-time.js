    ...
    return {
      ...
      link: function(scope, element, attrs, ngModel) {
        ...
        attrs.$observe('defaultDate', function(val) {
          return scope._defaultDate = (val != null) && moment(val).isValid() ? moment(val) : moment(scDateTimeConfig.defaultDate);
        });
        ...
        attrs.$observe('mindate', function(val) {
          if ((val != null) && moment(val).isValid()) {
            scope.restrictions.mindate = moment(val);
            scope.restrictions.mindate.hours(0);
            scope.restrictions.mindate.minutes(0);
            scope.restrictions.mindate.seconds(0);
            scope.restrictions.mindate.milliseconds(0);
            return scope.restrictions.mindate;
          }
        });
        attrs.$observe('maxdate', function(val) {
          if ((val != null) && moment(val).isValid()) {
            scope.restrictions.maxdate = moment(val);
            scope.restrictions.maxdate.hours(0);
            scope.restrictions.maxdate.minutes(0);
            scope.restrictions.maxdate.seconds(0);
            scope.restrictions.maxdate.milliseconds(0);
            return scope.restrictions.maxdate;
          }
        });
      ...
      controller: [
        '$scope', 'scDateTimeI18n', function(scope, scDateTimeI18n) {
          ...
          scope.setDate = function(newVal, save) {
            ...
            scope.date = newVal ? moment(newVal) : moment();
            scope.calendar._year = scope.date.year();
            scope.calendar._month = scope.date.month();
            scope.clock._minutes = scope.date.minutes();
            scope.clock._hours = scope._hours24 ? scope.date.hours() : scope.date.hours() % 12;
          ...
          scope.display = {
            ...
            main: function() {
              return $sce.trustAsHtml("TODO"); //$sce.trustAsHtml(scope._mode === 'date' ? _dateFilter(scope.date, 'd') : scope._hours24 ? _dateFilter(scope.date, 'HH:mm') : (_dateFilter(scope.date, 'h:mm')) + "<small>" + (_dateFilter(scope.date, 'a')) + "</small>");
            },
            sub: function() {
              if (scope._mode === 'date') {
          ...
          scope.calendar = {
            ...
            offsetMargin: function() {
              return (moment([this._year, this._month]).day() * 2.7) + "rem";
            },
            isVisible: function(d) {
              return moment([this._year, this._month, d]).month() === this._month;
            },
            isDisabled: function(d) {
              var currentDate, maxdate, mindate;
              currentDate = moment([this._year, this._month, d]);
            ...
            isPrevMonthButtonHidden: function() {
              ...
              return (date != null) && this._month <= date.month() && this._year <= date.year();
            },
            ...
            "class": function(d) {
              var classString;
              classString = '';
              if ((scope.date != null) && moment([this._year, this._month, d]).format("yyyy-MM-dd") === moment(scope.date).format("yyyy-MM-dd")) {
                classString += "selected";
              }
              if (moment([this._year, this._month, d]).format("yyyy-MM-dd") === moment().format("yyyy-MM-dd")) {
                classString += " today";
              }
              return classString;
            },
            select: function(d) {
              scope.date.year( this._year);
              scope.date.month(this._month);
              scope.date.day(  d);
            ...
            monthChange: function(save) {
              ...
              mindate = scope.restrictions.mindate;
              maxdate = scope.restrictions.maxdate;
              if ((mindate != null) && mindate.year() === this._year && mindate.month() >= this._month) {
                this._month = Math.max(mindate.month(), this._month);
              }
              if ((maxdate != null) && maxdate.year() === this._year && maxdate.month() <= this._month) {
                this._month = Math.min(maxdate.month(), this._month);
              }
              scope.date.year( this._year);
              scope.date.month(this._month);

              if (scope.date.month() !== this._month) {
                scope.date.day(0);
              }
            ...
            yearChange: function(save) {
              ...
              mindate = scope.restrictions.mindate;
              maxdate = scope.restrictions.maxdate;
              i = (mindate != null) && mindate.year() === scope.calendar._year ? mindate.month() : 0;
              len = (maxdate != null) && maxdate.year() === scope.calendar._year ? maxdate.month() : 11;
          ...
          scope.clock = {
            ...
            setAM: function(b) {
              ...
              if (b && !this.isAM()) {
                scope.date.hours(scope.date.hours() - 12);
              } else if (!b && this.isAM()) {
                scope.date.hours(scope.date.hours() + 12);
              }
              return scope.saveUpdateDate();
            ...
            isAM: function() {
              return scope.date.hours() < 12;
            }
          };
          scope.$watch('clock._minutes', function(val, oldVal) {
            if ((val != null) && val !== scope.date.minutes() && !isNaN(val) && (0 <= val && val <= 59)) {
              scope.date.minutes(val);
              return scope.saveUpdateDate();
            }
          });
          scope.$watch('clock._hours', function(val) {
            if ((val != null) && !isNaN(val)) {
              ...
              if (val !== scope.date.hours()) {
                scope.date.hours(val);
                return scope.saveUpdateDate();
              }
            }
          });
